...
public static class SampleRomData
{
    public static string GetSampleUtf8CartridgeTitle() => "｢ﾎ｣ abcｦｧｨ TEST123"; // don't pad here
    
    public static (int originalRomSizeBeforePadding, Data data) CreateSampleData()
    {
        ...
        while (data.RomBytes.Count < 0x8000)
        ...
        var overrideRomBytes = data.RomBytes.Select(rb =>
        {
        ...
        data.RomBytesOverrideProvider = () => overrideRomBytes;

        return (originalRomSizeBeforePadding, data);
    }
}