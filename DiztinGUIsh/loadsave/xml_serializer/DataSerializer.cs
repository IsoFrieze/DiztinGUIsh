...
sealed class RomBytesSerializer : ISerializer<RomBytes>
{
    ...
    private void DecodeCompression_Table1(ref List<string> lines)
    {
        for (int i = 0; i < lines.Count; ++i)
        {
            ...
            {
                lines[i] = lines[i].Replace(e.c, e.Pattern);
            }
        }
    }
    ...
    private void EncodeCompression_Table1(ref List<string> lines)
    {
        ...
        for (int i = 0; i < lines.Count; ++i)
        {
            ...
            {
                Debug.Assert(!lines[i].Contains(e.c));
                lines[i] = lines[i].Replace(e.Pattern, e.c);
            }
        }
    }
    ...
    private static List<FlagEncodeEntry> flagEncodeTable = new List<FlagEncodeEntry> {
